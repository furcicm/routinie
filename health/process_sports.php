...
?>
	...
	<div id="add-progress">
        ...
        $uid = $_SESSION['uid'];
        $subcategory = array();
        $query = mysql_query("SELECT `title`, SUM(`time`) AS s_time, SUM(`games`) AS s_games
            ...
            <div class="add-progress-row">
              ...
              <form class="add-progress-form" action="include/process_progress.php" method="POST" error=<?php echo $nr;?> >
                <div id="form-top">
                  ...
                  <input type="hidden" name="table" value="progress_sports" />
...
<?php 
    $empty = 1;
    $query = mysql_query("SELECT `title`, `time`, `games`  
                          FROM progress_sports   
                          WHERE `uid`='$uid' 
                          AND `done`=1
                          AND `active`=1
...
<?php }
    ...
    if ($empty) {
      echo "<div class='row-completed'>You haven't finished any $subcategory_s</div>";
    } ?>
	</div>
