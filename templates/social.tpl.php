<?php
  ...
  $title_add_new = $variables['title_add_new'];
  $table = $variables['table'];
?>
	
	<div id="title">Social - <?php echo ucfirst($subcat_pl); ?></div>
  ...
  <div id="add-new-progress" class="progress-title-social">
    ...
    <div class="add-new-social-subtitle subtitle-social-time">How much</div>
        ...
        <input type="hidden" name="table" value="progress_social" />
...
<?php 
    $uid = $_SESSION['uid'];
    $empty = 1;
    $query = mysql_query("SELECT `time`, `with`, `where`
                          FROM $table  
                          WHERE `uid`='$uid' 
                          AND `done`=0
                          AND `active`=1
