<?php
  ...
  $title_completed = $variables['title_completed'];
  $table = $variables['table'];
?>
	
	<div id="title">Enterteinment - <?php echo ucfirst($subcat_pl); ?></div>
  ...
  <div id="add-new-progress" class="progress-title-social">
    ...
    <div class="add-new-social-subtitle subtitle-social-time">How much</div>
		<div class="add-new-social-subtitle subtitle-social-place">Type</div>
		<form id="form-add-new-progress" action="include/process_progress.php" method="POST">
			<input id="add-progress-input" type="text" name="title-movie" autocomplete="off" class="input-social-whom"/>
			<span id="float-right">
				<select name="type-movie" class="movie-select">
