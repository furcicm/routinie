...
?>
	...
	<div id="title">Education - <?php echo ucfirst($variables['title']); ?></div>
	...
	<div id="add-progress">
        ...
        $sid = $variables['sid'];
        $books = array();
        $query = mysql_query("SELECT `title`, SUM(`page`) AS s_pages, SUM(`time`) AS s_time 
                              FROM $table
                              WHERE `uid`='$uid' 
                              AND `done`=0 
                              AND `active`=1
...
<?php 
    $empty = 1;
    $query = mysql_query("SELECT `title`, `time`, `page` 
                          FROM $table   
                          WHERE `uid`='$uid' 
                          AND `done`=1
                          AND `active`=1
    ...
    if ($query) {    
       while ($row = mysql_fetch_array($query)) { $empty = 0; ?>
        <div class="row-completed">
          <div class="completed-title"><?php echo "\"" . ucfirst($row['title']) . "\" - " . $row['page'] . " pages read in " . $row['time'] . " minutes."; ?></div>
